...
namespace VisWindow
{
    ...
    VCornerBox::VCornerBox(QWidget* parent) : QWidget(parent)
    {
    }

    void VCornerBox::paintEvent(QPaintEvent* e)
    {
        Q_UNUSED(e);
	...
	VRuleBar::VRuleBar(Qt::Orientation direction, QGraphicsView* view, QWidget* parent) :QWidget(parent), m_view(view), m_faceColor(0xFF, 0xFF, 0xFF)
    {
    ...
    void VRuleBar::SetRange(double lower, double upper, double max_size)
    {
        m_lower = lower;
        m_upper = upper;
        m_maxsize = max_size;
    ...
    void VRuleBar::UpdatePosition(const QPoint& pos)
    {
        m_lastPos = pos;
        update();
    }

    void VRuleBar::paintEvent(QPaintEvent* event)
    {
        Q_UNUSED(event);
        QPainter painter(this);
    ...
    void VRuleBar::DrawTicker(QPainter* painter)
    {
        int              i;
        int              width, height;
            ...
            for (cur = start; cur <= end; cur += subd_incr)
            {
                    ...
                    (label_spacing_px > 3 * digit_height || tick_index % 4 == 0 || cur == 0))
                {
                    if (qAbs((int)cur) >= 2000 && (((int)cur) / 1000) * 1000 == ((int)cur))
                        sprintf_s(unit_str, "%dk", ((int)cur) / 1000);
                    else
                        sprintf_s(unit_str, "%d", (int)cur);
                    if (m_direction == Qt::Horizontal)
                    {
...
#else
                        int w = fm.width(unit_str);
                        QRect textRect(-w / 2, -VRULER_SIZE / 2, w, VRULER_SIZE);
                        painter->save();
                        painter->translate(4, pos + w / 2 + 2);
                        painter->rotate(90);
    ...
    void VRuleBar::DrawPos(QPainter* painter)
    {
        int  x, y;
        int  width, height;
