...
{
	strategy_.CreateShapeModel(templateImg, numLevels, angleStart, angleExtent, angleStep, optimization, metric, contrast, minContrast);
}
...
template<typename Strategy>
inline void ShapeModel<Strategy>::FindShapeModel(const Image& image, double angleStart, double angleExtent, double minScore, int64_t numMatches, double maxOverlap, const char* subPixel, int64_t numLevels, double greediness, Tuple* row, Tuple* column, Tuple* angle, Tuple* score) const
{
}
